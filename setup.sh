...
# This script helps new developers set up the project after cloning
...
if [[ "$RUBY_VERSION" != "3.2.1" ]]; then
    echo "⚠️  Warning: Expected Ruby 3.2.1, found $RUBY_VERSION"
...
if [ ! -f "config/database.yml" ]; then
    ...
    echo "ℹ️  config/database.yml already exists"
fi
...
if [ ! -f "config/application.yml" ]; then
    ...
    echo "ℹ️  config/application.yml already exists"
fi
...
if [ ! -f "config/secrets.yml" ]; then
    ...
    echo "ℹ️  config/secrets.yml already exists"
fi
